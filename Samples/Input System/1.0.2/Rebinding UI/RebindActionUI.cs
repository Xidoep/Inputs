...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public GuardarEvent guardarEvent
        {
            get
            {
                if (m_guardatEvent == null)
                    m_guardatEvent = new GuardarEvent();
        ...
        //---(XIDO)
        bool CheckDuplicatedBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            InputBinding newBindings = action.bindings[bindingIndex];
            foreach (var binding in action.actionMap.bindings)
            {
                if(binding.action == newBindings.action)
                {
                    continue;
                }
